...
<p align="center">
  ...
  visible root, on-demand hydrate, 파일 및 폴더 단위 residency, tray 중심 복구 흐름, 그리고 셸·CLI·Dolphin이 공유하는 하나의 daemon 상태를 제공합니다.
</p>
...
- 경로를 직접 입력하지 않아도 되는 debounced 전체 검색, residency 필터, 빈 결과/오류 구분, 파일 및 폴더 direct action을 갖춘 Explorer 화면
...
- checksum 검증, 기존 설치 업그레이드 확인, 릴리즈 워크플로우의 launcher/upgrade smoke test를 포함한 `curl ... | bash` 설치 경로

## 운영 표면

- `Overview`: 다음 액션, compact runtime inspector, recent activity를 한 페이지에 모읍니다
- `Explorer`: path-state 데이터를 탐색하고, online-only 항목을 계속 보이게 유지하며, 진짜 빈 폴더와 backend 오류를 구분하고, residency 필터와 파일/폴더 action을 제공합니다
...
- Explorer 페이지는 unavailable / error / empty 상태를 분리한 searchable path-state view와 bulk / row-level residency action을 제공합니다
